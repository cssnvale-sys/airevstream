...
- **Manual experiment winner declaration** (Session 50): `POST /api/v1/experiments/[id]/declare-winner` lets operators complete experiments that need human judgement (small sample size, qualitative trade-offs). Validates variant ownership, rejects invalid status transitions, records `declaredWinner` metadata in `config`. Closes Wave-1 A4 blocker.
- **Budget-alert persistence** (Session 50): `GET /api/v1/budgets/check` now writes `Alert` rows on threshold and exceeded crossings, de-duplicated by metadata `alertKey` within a 24 h window. Previously the cost/alerts page stayed empty no matter how bad the overrun was. Closes Wave-1 A4 blocker.
- **Storyboard persistence on content creation** (Session 50): `POST /api/v1/content` wraps entity creation in a `$transaction`; when shots are supplied, creates a `Storyboard` row plus `StoryboardShot` rows with cumulative `startSec/endSec` timing, `shotspec` with `promptBlocks` + `duration`, and `keyframeUrls`. Production-pipeline workers now actually see the shot list instead of it being orphaned inside `generationParams`. Closes Wave-1 A2 blocker.
- **`pickSafeMessage()` helper** (Session 50): `apps/web/src/lib/safe-messages.ts` strips the field-path prefix `formatZodErrors` adds (`"password: …"`) so auth-page `safeMessages` allowlists match. Register, reset-password, and forgot-password pages now surface the real validation error instead of the generic fallback. Closes Wave-1 A1 blocker.
- **Fresh-machine bootstrap infrastructure** (Session 49): New `scripts/doctor.sh` preflight (checks Docker, Node/npm versions, required + optional CLI tools, host port collisions, .env presence, required secrets populated) and `scripts/bootstrap.sh` one-shot bringup (env validation → `docker compose up -d` → wait for healthy → `prisma migrate deploy` → `turbo build` → ready banner). `make doctor`, `make bootstrap`, `make reset` targets wire these into the project Makefile. Optional-tool checks (ffmpeg, python3, jq) surface `brew install` hints without failing.
- **Fail-fast environment validation** (Session 49): `packages/shared/src/config.ts` now parses `process.env` through a strict Zod schema at startup in every process (web, services, workers). Missing or placeholder secrets (JWT_SECRET, ENCRYPTION_KEY, DATABASE_URL, REDIS_URL, S3 creds) throw immediately with a clear list of what's missing, instead of silently falling back to defaults that later crash at first use. Applies in dev and prod — no silent "it'll probably work" path.
...
### Fixed
- **Ollama `think: false` is the default** (Session 49): `packages/ai-client/src/providers/ollama.ts` now passes `think: request.think ?? false` to `ollama-js` on `generateText`, `generateChat`, and `streamChat`. Added `stripThinkingTags()` helper that removes `<think>…</think>` blocks from `message.content` defensively in case a model ignores the flag; `streamChat` uses a cross-chunk state machine so partial tags don't leak mid-stream. `packages/ai-client/src/types.ts` — `TextRequest` / `ChatRequest` gained `think?: boolean` so callers can opt into reasoning mode per request (e.g. complex planning). qwen3:8b content generation dropped from ~4 min to expected <30 s. See D132 + KI-090.
- **Workers no longer exit silently on uncaught exception** (Session 49): `workers/src/index.ts` registers top-level `uncaughtException` (logs full stack via Pino, exits 1 with a 100 ms flush delay so a supervisor can restart) and `unhandledRejection` (logs only — BullMQ job failures surface via per-queue `on('failed')` and should not kill the host). Previously an unhandled exception in a worker processor terminated the node process with zero output, leaving jobs stranded in `active` with stale locks. See KI-091.
- **Workers `MaxListenersExceededWarning` silenced** (Session 49): `workers/src/index.ts` — `process.setMaxListeners(20)` (was default 10). Nine BullMQ Worker instances each install a SIGTERM/SIGINT exit listener, tipping over the default and masking any real listener leaks behind a cosmetic warning. See KI-092.
- **Ollama default model now honors `OLLAMA_DEFAULT_MODEL` env** (Session 49): `packages/ai-client/src/providers/ollama.ts`, `packages/ai-client/src/index.ts`, and `packages/ai-client/src/registry.ts` all resolve the Ollama tag in the order: explicit `request.model` → `process.env.OLLAMA_DEFAULT_MODEL` → seeded `AiService.capabilities.defaultModel` (registry path only) → compiled fallback `'qwen3:8b'`. Evaluation happens at request time so PM2 per-worker env is respected. `registry.getModelFromCapabilities()` specifically short-circuits the DB capability lookup for `provider === 'ollama'` when the env var is set, so operators with a larger pulled tag (e.g. `qwen3.5:122b` on a 512 GB Mac Studio) no longer have to hand-edit the DB seed row. `scripts/doctor.sh` reads the same env var when validating "default model installed," so the diagnostic matches what the code will actually try. `.env.example` documents the override with a note on how to list installed tags. See D131 + KI-089.
- **Service port wiring matches operator defaults** (Session 49): `services/{workflow-engine,ai-assistant,production-pipeline}/src/index.ts` now read their own named env vars (`WORKFLOW_ENGINE_PORT`, `AI_ASSISTANT_PORT`, `PRODUCTION_PIPELINE_PORT`) before falling back to `PORT` and then to the default. The workflow-engine default is now `3011` (was `3001`) to stop colliding with mission-control / openclaw. `packages/shared/src/constants.ts::PORTS.WORKFLOW_ENGINE`, `ecosystem.config.js` PM2 env, `scripts/doctor.sh` port table + Redis probe, `.env.example` defaults, `CLAUDE.md`, and `.claude/rules/03-monorepo-map.md` updated to match. Doctor now parses `REDIS_URL` to determine the host port to probe, so operators who override Redis never get a stale "Redis unreachable" warning.
- **minio-init CORS heredoc** (Session 49): `docker-compose.yml` `entrypoint: >` folded scalar collapsed the heredoc that configured the S3 bucket CORS policy into a single line, which MinIO's `mc` client parsed as `cat: '{': No such file or directory` and exited 1, breaking `make bootstrap`. Replaced heredoc with a single-line `echo '{json}' > /tmp/cors.json` that survives YAML folding. Bootstrap now completes end-to-end on a clean checkout.
- **Remotion dev script no longer hijacks browser** (Session 49): `remotion/package.json` `dev` script previously ran `remotion studio`, which auto-launches Remotion Studio on :3004 in the default browser every time `turbo dev` or `npm run dev` runs at the repo root. Split into `dev: tsc --watch --preserveWatchOutput` (silent type-watch, used by turbo) and opt-in `studio: remotion studio src/Root.tsx` (operator runs manually when editing compositions). Stops the landing page from appearing instead of the dashboard on fresh startup.
- **System page port labels** (Session 49): `apps/web/src/app/system/page.tsx` DEFAULT_SERVICES entries for Workflow Engine and Redis now display `localhost:3011` and `localhost:6389` to match the new host-port assignments (moved to avoid colliding with openclaw on 3001 and Homebrew redis on 6379).
- **Env defaults for ports in use** (Session 49): `.env` `WORKFLOW_ENGINE_PORT=3011` and `REDIS_URL=redis://localhost:6389` documented as the default so operators don't have to hand-edit after first clone.
...
- **Design system color migration** (Session 47): ~130 hardcoded Tailwind color classes across 25+ files migrated to accent-* tokens. Covers status indicators, preset families, platform colors, chart colors, budget bars, and all cinema components. Analytics CHART_COLORS + tooltipStyle hex values replaced with CSS variable references.
- **WCAG accessibility** (Session 47 cont.): role="dialog" aria-modal="true" on 6 modals, scope="col" on 103 table headers across 12 files, focus-visible rings on toggle/tab/action buttons, noValidate on 14 forms, sticky table headers on 3 tables, type="button" on 6 non-submit buttons.
- **Legacy quality tier cleanup** (Session 47 cont.): Removed last 'quick' tier reference from cost-preview-panel props.
