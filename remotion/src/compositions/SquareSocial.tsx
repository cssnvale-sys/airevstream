...
/**
 * SquareSocial — 1:1 square video composition (1080x1080, 30fps)
 *
 * Designed for Instagram feed posts, Facebook feed, and other square-format
 * social media placements. Uses the H.I.C.C. framework for beat timing
 * and visual rhythm, same as ShortFormVideo.
 *
 ...
 * 4. H.I.C.C. Section indicator (subtle)
 * 5. Script text for current section
 * 6. Custom text overlays
 * 7. Audio visualization
 * 8. Audio track
 */
export const SquareSocial: React.FC<SquareSocialProps> = ({
  title: _title,
  script,
  shots,
  audioUrl,
  ...
  const timings = beatTimings.length > 0 ? beatTimings : DEFAULT_SQUARE_BEAT_TIMINGS;
...
const ScriptText: React.FC<ScriptTextProps> = ({
  ...
  // Square format: balanced margins, centered text
  const margin = 60;
  const fontSize = 32;
